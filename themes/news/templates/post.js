/* ============================================================
   ...
   Layout baca berita: kategori, judul besar, byline, foto fitur,
   isi artikel, tag, artikel terkait. Mendukung sidebar opsional.
...
module.exports = function post(ctx) {
  ...
  var post = ctx.post, related = ctx.related;
  ...
  var relatedHtml = (related && related.length)
    ? '\n    <section class="related"><div class="container">' +
      '<h2 class="related-title">Baca Juga</h2>' +
      '<div class="related-grid">' +
      related.map(function (p) {
        return (
          '<a href="' + attr(U.url(p.permalink)) + '" class="related-card">' +
          (p.featuredImage ? '<div class="related-thumb"><img src="' + attr(U.url(p.featuredImage)) + '" alt="' + attr(p.meta.title) + '" loading="lazy"></div>' : "") +
          '<div class="related-info">' +
          (p.meta.category ? '<span class="related-cat">' + esc(p.meta.category) + "</span>" : "") +
  ...
  if (blocks.length) {
    ...
    article =
      ...
      (featuredFig ? '\n      <div class="container post-hero-wrap">' + featuredFig + "</div>" : "") +
