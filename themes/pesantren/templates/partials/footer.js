/* ============================================================
   partials/footer.js — Footer situs + widget (TEMA Pesantren)
   Bagian atas: widget footer (bila ada) dari data inti (ctx.site).
   Bagian utama: kolom brand, tautan cepat (dari menu), dan kontak
   (alamat/telepon/WhatsApp/email dari Customizer "Lokasi & Kontak").
...
// Tautan cepat (maks 6) dari menu navigasi.
...
module.exports = function footer(ctx) {
  ...
  // Kolom kontak: hanya tampilkan baris yang terisi.
  var contactItems = [];
  if (k.address) contactItems.push('<li class="fc-addr">' + esc(k.address) + "</li>");
  if (k.phone) contactItems.push('<li><a href="tel:' + attr(k.phone.replace(/\s+/g, "")) + '">' + esc(k.phone) + "</a></li>");
  if (k.waLink) contactItems.push('<li><a href="' + attr(k.waLink) + '" target="_blank" rel="noopener">WhatsApp</a></li>');
  if (k.email) contactItems.push('<li><a href="mailto:' + attr(k.email) + '">' + esc(k.email) + "</a></li>");
  if (k.hours) contactItems.push('<li class="fc-hours">' + esc(k.hours) + "</li>");
  var contactCol = contactItems.length
    ? '<div class="footer-col footer-contact"><h4 class="footer-col-title">Kontak</h4><ul class="footer-links">' + contactItems.join("") + "</ul></div>"
    : "";
  ...
  return (
    ...
    '\n        <div class="footer-brand-name"><span class="site-logo-mark" aria-hidden="true">☾</span>' + esc(config.title) + "</div>" +
