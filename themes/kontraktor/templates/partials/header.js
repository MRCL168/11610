/* ============================================================
   partials/header.js — Header dan navigasi tema Kontraktor
   Navigasi tetap membaca config.nav dari inti, CTA membaca data tema.
   ============================================================ */
...
module.exports = function header(ctx) {
  ...
  function navItemHtml(n) {
    ...
    var sub = children.map(function (c) {
      return '<a class="submenu-link" href="' + attr(navHref(U, c.url || "/")) + '">' + esc(c.label) + '</a>';
    }).join("");
  ...
  var cta = profile.headerCta.enabled
  ...
  var brand = config.logo
    ...
    : '<a href="' + attr(U.url("/")) + '" class="site-logo"><span class="logo-mark">▰</span><span>' + esc(config.title || "GitCMS") + '</span></a>';
